...
return (
    <footer ref={footerRef} className="relative overflow-hidden gradient-premium">
      {/* Top gradient accent */}
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 pt-16 lg:pt-20 pb-8">
        ...
        >
          ...
          <motion.div variants={fadeUp} className="lg:col-span-5">
            ...
            {/* Newsletter */}
            <div className="mb-6">
              <h4 className="font-heading font-semibold text-[0.8125rem] tracking-widest uppercase text-white/70 mb-2">
                {t('footer.newsletter.title')}
              </h4>
              <p className="text-sm text-white/70 mb-4">{t('footer.newsletter.desc')}</p>
    ...
    <motion.div
          ...
          className="flex items-center gap-2 px-4 py-3 rounded-xl bg-[var(--color-brand-blue)]/15 border border-[var(--color-brand-blue)]/25"
              ...
              <p className="text-xs text-white/55 mt-2">{t('footer.newsletter.privacy')}</p>
            </div>

            {/* Language Switcher */}
            <div className="relative">
              <span className="text-xs text-white/65 uppercase tracking-wider mr-3">{t('footer.language.label')}</span>
        <button
          ...
          className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-sm text-white/85 hover:text-white hover:border-white/15 transition-all cursor-pointer"
        >
        ...
        >
          {langs.map(l => (
            <a
              ...
              className={cn(
                ...
                l.code === lang ? "text-[var(--color-brand-blue)]" : "text-white/80 hover:text-white hover:bg-white/[0.04]"
              )}
            >
                      <span>{l.flag}</span>
          ...
          <motion.div variants={fadeUp} className="lg:col-span-2">
            <h4 className="font-heading font-semibold text-[0.8125rem] tracking-[0.05em] uppercase text-white/70 mb-5">
              {t('footer.solutions.title')}
            </h4>
            <ul className="space-y-3">
              {solutionLinks.map(link => (
                <li key={link.href}>
                  <a href={link.href} className="text-sm text-white/75 hover:text-white hover:pl-1 transition-all duration-200 block">
                    {t(link.label)}
                  </a>
                </li>
              ))}
          ...
          <motion.div variants={fadeUp} className="lg:col-span-3">
            ...
            <ul className="space-y-3">
              <li>
                <a href="mailto:hello@cliltutors.com" className="text-sm text-white/75 hover:text-white transition-colors">
                  {t('footer.email')}
                </a>
              </li>
              <li>
                <a href={`/${lang}/contact`} className="text-sm text-white/75 hover:text-white transition-colors">
                  {t('nav.contact')}
                </a>
              </li>
              <li>
        ...
        <a
          ...
          aria-label="LinkedIn"
                className="w-9 h-9 rounded-[10px] bg-white/[0.05] border border-white/[0.08] flex items-center justify-center text-white/65 hover:bg-[var(--color-brand-blue)]/15 hover:border-[var(--color-brand-blue)]/30 hover:text-[var(--color-brand-blue)] hover:-translate-y-0.5 transition-all duration-300"
              >
    ...
    transition={prefersReducedMotion ? { duration: 0 } : { delay: 0.6, duration: 0.5 }}
        ...
        >
          <p className="text-sm text-white/65">{t('footer.copyright')}</p>
          <div className="flex gap-6">
            <a href="#" className="text-sm text-white/65 hover:text-white/90 transition-colors">{t('footer.privacy')}</a>
            <a href="#" className="text-sm text-white/65 hover:text-white/90 transition-colors">{t('footer.terms')}</a>
          </div>
        </motion.div>
      </div>
    </footer>
